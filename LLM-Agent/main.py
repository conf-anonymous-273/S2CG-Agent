...
import torch
from decision import LLMDecisionAgent
...
class MultiAgentSystem:
    ...
    def run(self, model, tokenizer, device, iterations=100):
        ...
        # 现在已经得到了代码，需要让LLM自行决定是否执行三个测试。
        # 每个测试只能执行一次。
        decision_agent = LLMDecisionAgent(self.llm)
        test_choices = ['unit_test', 'static_analysis', 'fuzzing']
        test = decision_agent.decide(
            self.entry['Prompt'], self.code, test_choices)
        if test == 'unit_test':
            func_test_status = self.unit_test()
            test_choices.remove('unit_test')
            test = decision_agent.decide(
                self.entry['Prompt'], self.code, test_choices)
            if test == 'static_analysis':
                static_analysis_status = self.static_analyze()
                test_choices.remove('static_analysis')
                test = decision_agent.decide(
                    self.entry['Prompt'], self.code, test_choices)
                if test == 'fuzzing':
                    fuzzing_test_status = self.fuzzing(iterations)
                    test_choices.remove('fuzzing')
            if test == 'fuzzing' and 'fuzzing' in test_choices:
                fuzzing_test_status = self.fuzzing(iterations)
                test_choices.remove('fuzzing')
                test = decision_agent.decide(
                    self.entry['Prompt'], self.code, test_choices)
        ...
        if test == 'static_analysis' and 'static_analysis' in test_choices:
            static_analysis_status = self.static_analyze()
            test_choices.remove('static_analysis')
            test = decision_agent.decide(
                self.entry['Prompt'], self.code, test_choices)
        ...
        if test == 'fuzzing' and 'fuzzing' in test_choices:
            ...
            if test == 'static_analysis':
                ...
                if test == 'unit_test':
                    func_test_status = self.unit_test()
                    test_choices.remove('unit_test')
            if test == 'unit_test' and 'unit_test' in test_choices:
                func_test_status = self.unit_test()
                test_choices.remove('unit_test')
                test = decision_agent.decide(
                    self.entry['Prompt'], self.code, test_choices)
...
# 2. 定义一个函数读取 JSONL 文件
    ...
    llm = OpenAI_LLM(openai_api_key, 'gemini-1.5-pro-latest')
