...
class LLMFunctionalTestAgent(FunctionalTestAgentInterface):
    def __init__(self, entry, llm):
        self.entry = entry
        self.llm = llm
        prompt_path_testcase = "./prompts_fewshot/func_testcase_prompt.txt"

        with open(prompt_path_testcase, "r") as f:
            self.construct_few_shot_prompt_testcase = f.read()

    def call_chatgpt_testcase(self, prompt) -> str:
        text = f"""{self.construct_few_shot_prompt_testcase}
...
"""
        ...
        test_case = self.llm.generate(text)
        test_case = preprocess_string(test_case, 'json')
        return test_case
