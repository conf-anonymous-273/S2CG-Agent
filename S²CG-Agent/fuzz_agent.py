...
class TesterFuzzAgent:
    ...
    def call_chatgpt_fuzzing_tester(self, prompt):
        _str = '''For example:{"parameter1": 1.0, "parameter2": 'test'}'''
        text = f'''You are a code tester specialized in fuzzing.
Create initial input for fuzzing testing the incomplete function. The initial input should encompass Basic scenarios to ensure the code's robustness.
**Instructions**:
    - Implement a comprehensive set initial input based on the given requirements for the incomplete function.
    - The format should only be a JSON string, do not generate any comments. {_str}
    - Only output one initial input
...
'''
        try:
            new_completion = self.llm.generate(text)
